...
def home():
	...
	for i in range(num):
		try:
			annrec = Announcement_recipient(announcement_id = aid[i], recipient = recipient[i])
	...
	for i in range(num):
		try:
			poll_rec = Poll_recipient(poll_id = pid[i], recipient = poll_recipient[i], completed = completed[i], choice = poll_choice[i])
	...
	for i in range(num):
		try:
			task_rec = Task_recipient(task_id = tid[i], recipient = task_recipient[i], completed = task_completed[i])
